...
@info "Running Lev₂³ + Lev₃³ - λ·Δ sum of squares decomposition for " G
...
certified, λ = solve_in_loop(
    ...
    wd,
    varP;
    logdir = "./log/Sp($(2N),Z)/r=$HALFRADIUS/Level-$(UPPER_BOUND)Δ",
...
if certified && λ > 0
    @info "Certified result: in group $G we have Lev₂³ + Lev₃³ -λΔ ≥ 0" λ R =
        HALFRADIUS
else
